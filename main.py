...
if __name__ == "__main__":
    ...
    try:
        ...
        sql = SqlConnector("database.db")
        last_error = sql.get_error_notifications()
        if last_error is not None and last_error[2] == str(e) and datetime.datetime.now(datetime.timezone.utc) - datetime.datetime.strptime(last_error[1], "%Y-%m-%d %H:%M:%S").replace(tzinfo=datetime.timezone.utc) < datetime.timedelta(minutes=0):
            print("Error already logged.")
        else:
            print("Logging error...")
            sql.insert_error_notification(str(e))
                ...
                Thread(
                    ...
                    kwargs={
                        "json": {
                            ...
                            "message": f"{e}. The application has stopped.",
                            "authenticity_key": PUSH_KEY
                        },
                        "headers": {"Content-Type": "application/json"}
                    }
